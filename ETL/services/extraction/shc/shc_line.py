...
def shc_line(line:any, elm: any, Shc:any, direction:str, terminal:str,data: list[dict[str:float | str]]) -> dict[str, float]:

    Shc.iopt_mde = 3 # Completo
    Shc.iopt_allbus = 0 # User selection
    Shc.shcobj = elm # Fault in the element
    terminal_dict = {'bus1':0,'bus2':1}
    Shc.ipot_dfr = terminal_dict[terminal] if direction == 'reverse' else 1 - terminal_dict[terminal]

    percentages = [1] + list(range(5,100,5)) + [99]
        ...
        for p in percentages:
            ...
            for r in r_faults:
                ...
                row = {
                    ...
                    'dir':direction}
                print(f'            fault type: {fault_type} {p}% Rf: {r}', end='\r', flush=True)
                data.append(row)